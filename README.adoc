...
This macro generates code (i.e., source) blocks by importing any code files found in one or more languages (currently XML, Java, Kotlin, or Groovy) from preconfigured locations.
...
The location of the includes are defined by AsciiDoc attributes named `include-code-<language>-template`, where `<language>` is the lowercase name of the language (e.g., `java`).
...
[,properties]
----
include-code-java-template=example$java/${adoc_filename}/${adoc_target}${lang_ext}
----
...
| `adoc_path`
| The path of the source AsciiDoc file, with the file name removed and all `-` characters removed (to ensure it is a valid Java identifier).
| `org/spring`
...
| `include-<lang>`
| The asciidoc attribute that defines the base path for the target language, where `<lang>` is the lowercase name of the language.
For example, the value of the asciidoc attribute named `include-java` defines the base path for Java files.
| `example$src/main/java/`
|===

NOTE: The `Example Value` column is computed using the <<include-code-example,Example>> AsciiDoc source and <<include-code-java-template-example,include-code-java-template>>.
