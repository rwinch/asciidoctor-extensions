...
function register (registry) {
  if (!registry) return this.register('springio/code-folding', createExtensionGroup())
  registry.$groups().$store('springio/code-folding', toProc(createExtensionGroup()))
  return registry
}
...
function isEmpty (source) {
  ...
  if (source['$nil?'] && source['$nil?']()) return true
  const s = source.$to_s && typeof source.$to_s === 'function' ? source.$to_s() : String(source)
  return (typeof s === 'string' && /^\s*$/.test(s)) || s === ''
}
