    ...
    this.blockMacro(function () {
      ...
      this.process((parent, target, attrs) => {
        ...
        langs ??= [['java'], ['kotlin', '.kt'], ['groovy'], ['xml']].reduce((accum, [lang, ext]) => {
          ...
          const template = doc.getAttribute(templateAttrName(lang), defaultTemplate)
          const required = requiredLangs.includes(lang)
          if (template) accum.push({ name: lang[0].toUpperCase() + lang.slice(1), lang, ext: ext || '.' + lang, base: { attrName: includeLangAttrName, value: includeLang }, template, required })
          return accum
        }, [])
        ...
        if (notConfiguredRequiredLangs.length) {
          const missingAttrNames = notConfiguredRequiredLangs.map((lang) => templateAttrName(lang)).join(', ')
        ...
        const resources = langs.reduce((accum, { name, lang, ext, base, template, required }) => {
          const templateVars = {
...
function templateAttrName(lang) {
